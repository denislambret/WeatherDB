...
) ENGINE = InnoDB AUTO_INCREMENT = 12685 DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */
;
--
...
DELIMITER;
;
CREATE DEFINER = `dev` @`%` PROCEDURE `Get_Daily_Stats`(IN `my_date` DATE, IN `loc` INT) BEGIN # Daily stats from Records table
#-------------------------------------------------------------------------------------------------------
...
DELIMITER;
;
CREATE DEFINER = `dev` @`%` PROCEDURE `Get_Hourly_Stats`(IN `my_date` DATE, IN `loc` INT) BEGIN # Hourly stats from Records table
#-------------------------------------------------------------------------------------------------------
SELECT date_timestamp,
  #	CASE 
  ...
  ROUND(sum(rain_1h), 2) as sumRain,
  ROUND(sum(snow_1h), 2) as sumSnow
FROM RawRecords #WHERE (DATE(date_timestamp) = '2021-08-10') AND id_location = loc
...
DELIMITER;
;
CREATE DEFINER = `dev` @`%` PROCEDURE `Get_Monthly_Stats`(IN `my_date` DATE, IN `loc` INT) BEGIN # Monthly stats from Records table
#-------------------------------------------------------------------------------------------------------
select date_timestamp,
...
DELIMITER;
;
CREATE DEFINER = `dev` @`%` PROCEDURE `Set_Daily_Stats`(my_date DATE, loc INT) BEGIN
select null,
  my_date,
  ...
  round(sum(snow_1h), 2) as sumSnow
FROM Records
WHERE (day(date(date_timestamp)) = day(curdate()))
  AND id_location = 6
GROUP BY day(date(date_timestamp));
END;
;
...
DELIMITER;
;
CREATE DEFINER = `dev` @`%` PROCEDURE `Set_Hourly_Stats`(my_date DATE, loc INT) BEGIN
...
FROM Records
WHERE (DATE(date_timestamp) = my_date)
  AND id_location = loc #WHERE (DATE(date_timestamp) = my_date) AND id_location = loc
GROUP BY CASE
...
;
-- Dump completed on 2021-08-31 23:40:16